...
def find_test_files_for_module(module_path: str) -> list[str]:
    ...
    if len(parts) >= 2:
        ...
        if any(
            ...
            for keyword in ["eval", "validate", "parse"]
        ):
...
def get_test_category_for_module(module_path: str) -> str:
    ...
    if (
        "security" in str(path)
        or "validate" in module_path
        or "parse" in module_path
    ):
        return "security"
...
def run_mutation_testing_on_file(
    ...
    try:
        ...
        stats = {"killed": 0, "survived": 0, "timeout": 0, "suspicious": 0}
        ...
        return (
            False,
            f"{stats['survived']} mutants survived ({survival_rate:.1f}%) ❌",
            stats,
        )
...
def generate_survival_report() -> None:
...
def suggest_test_improvements(file_path: str, stats: dict[str, int]) -> None:
...
def main() -> int:
    ...
    if args.all:
        files_to_test = [
            str(f)
            for f in Path("energy_transformer").rglob("*.py")
            if not f.name.startswith("__")
            and not f.name.endswith("__init__.py")
        ]
    elif args.files:
    ...
    if total > 0:
        ...
        print(
            f"  Killed: {total_stats['killed']} ({total_stats['killed'] / total * 100:.1f}%)"
        )
        print(
            f"  Survived: {total_stats['survived']} ({total_stats['survived'] / total * 100:.1f}%)"
        )
